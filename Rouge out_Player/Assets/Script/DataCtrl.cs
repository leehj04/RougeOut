...
public class DataCtrl : MonoBehaviour
{
    ...
    public void ArmorCtrl(bool _whenOffEquip, float _armor)
    {
        if(_whenOffEquip) DataCtrl.instance.playerData.armor -= _armor; //장비를 해제할 때
        else DataCtrl.instance.playerData.armor += _armor;              //장비를 장착할 때 또는 다른 이유로 방어력이 증가 할 때.
        float temp_armor = DataCtrl.instance.playerData.armor;
        DataCtrl.instance.playerData.finalArmor = temp_armor / (temp_armor + 100);
    }
}
